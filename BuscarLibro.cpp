...
BuscarLibro::BuscarLibro(VentanaDos* ventanaAnterior, QWidget *parent)
    : QWidget(parent), ventanaDos(ventanaAnterior) {
    ...
    mainLayout->setMenuBar(menuBar);  // Agregar barra de menú al layout principal
    ...
    //tablaResultados->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);

    tablaResultados->setEditTriggers(QAbstractItemView::NoEditTriggers);

    tablaResultados->setSelectionMode(QAbstractItemView::SingleSelection);           // Selección individual
    tablaResultados->setSelectionBehavior(QAbstractItemView::SelectItems);           // Seleccionar celda, no fila
    tablaResultados->setTextElideMode(Qt::ElideNone);                                // No recortar texto
    tablaResultados->setContextMenuPolicy(Qt::ActionsContextMenu);                   // Para menú contextual si lo deseas
...
tablaResultados->addAction(copiarCelda);
    ...
    connect(inputBusqueda, &QLineEdit::textChanged, this, &BuscarLibro::buscarLibros);
    connect(comboEstado, &QComboBox::currentTextChanged, this, &BuscarLibro::buscarLibros);

    buscarLibros();
}

void BuscarLibro::buscarLibros() {
